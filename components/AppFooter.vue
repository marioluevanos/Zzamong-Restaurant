<template>
    <footer class='app-footer' v-if='location'>
        <address class='address'>
            ...
            <a class='link' :href='`tel:${location.phone}`' target='_blank'>{{formatPhoneNumber(location.phone)}}</a>
        </address>
        <div class='social'>
            <a :href='link.href' class='icon-link' target='_blank' v-for='link in social' :key='link.href'>
...
.app-footer {
    position: absolute;
    height: vw(var(--app-footer-height));
    bottom: 0;
    left: vw(var(--gutter));
    right: vw(var(--gutter));
    display: flex;
