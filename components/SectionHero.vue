<template>
    <ParallaxContainer tag='section' class='hero' data-id='home'>
        <figure class='hero-bg' ref='bg'>
            <ParallaxElement tag='img' :factor='1' :compensate='false' :src='hero.image' alt='Background Image'/>
        </figure>
        <ParallaxElement class='hero-content' :factor='-0.5' :compensate='false'>
...
export default {
    ...
    methods: {
        animateSection() {
            ...
            const title = new SplitText(this.$refs.title, { type: 'chars words', wordsClass: 'word' })
    ...
    mounted() {
        this.animateSection(); // Instead of the scrollObserver calling the animation, this is getting overrid with this mounted hook
    }
}
</script>
...
.hero-bg {
    ...
    top: 0; 
    right: 0; 
    bottom: vw(var(--app-footer-height)); 
    width: 100%;
    left: 0;
