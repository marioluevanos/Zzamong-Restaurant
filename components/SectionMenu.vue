<template>    
    <section class='menu' data-id='menu' v-if='menus'>
        <ParallaxContainer class='menu-static-column'>
            <ParallaxElement tag='h2' class='menu-title'>{{menus.title}}</ParallaxElement>
            <ParallaxElement 
                class='menu-filters'
                :factor='-1'
                :top='menuCssTop' 
            ...
            <figure class='menu-type-image'>
                <img v-for='type in menuTypeImages' 
                    :src='type.image' 
                    :key='type.id' 
                    :class='{ 
        ...
        <ul class='menu-types' ref='menuTypes'>
            <li 
                ...
                class='menu-type'>
                <img class='menu-type-preload' :src='menu.image' :data-image='menu.image' />
                <header class='menu-type-header'>
                    <h4 class='menu-type-title' v-html='menu.type'/>
                    <p class='menu-type-description' v-if='menu.description' v-html='menu.description'/>
                </header>
                <ul class='items'>
                    <li class='item' v-for='item in menu.items' :key='item.id'>
                        ...
                        <p class='item-data description' v-html='item.description' v-if='item.description'/>
                    </li>
                </ul>
            </li>
        </ul>
        <a class='hero-cta-btn' tabindex='-1' :href='hero.button1URL' target='_blank' rel='noopener'>
            <AppButton class='primary'>{{hero.button1Text}}</AppButton>
        </a>
    </section>
</template>
...
export default {
    ...
    methods: {
        ...
        toggleMenuClass(target) {
            Array.from(this.$refs.filters.$el.children).forEach(child => child.classList.remove('active'))
        ...
        setMenuCssTop() {
            this.menuCssTop = parseInt(getComputedStyle(document.documentElement).getPropertyValue(`--nav-height`)) + 20 || 0;
        }
    },
    mounted() {
        this.setMenuCssTop();
...
.menu-type-image {
    ...
    img {
        ...
        transition: 
            transform 0s ease-in 0.6s,
            clip-path 0s ease-in 0.6s;
    }
    img.previous {
