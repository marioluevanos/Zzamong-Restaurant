<template>    
    <ParallaxContainer 
        ...
        <ParallaxElement :factor='-0.1' tag='header' :top='false' class='press-header main-header'>
            <h3 ref='title' class='press-title'>{{press.title}}</h3>
        </ParallaxElement>

        <ParallaxContainer class='logos' ref='logos'>
            <AppIcon 
                type='corner' 
                :class='{[corner]: true}' 
                v-for='corner in ["top", "right", "bottom", "left"]' 
                :key='corner'/>
            <ParallaxElement class='logo' tag='figure' :compensate='false' :factor='-0.5' v-for='logo in press.logos' :key='logo'>
                <img :src='logo' alt='Logo'/>
            </ParallaxElement>
        </ParallaxContainer>

        <ParallaxContainer tag='ul' class='links' ref='links'>
            <ParallaxElement tag='li' :factor='-0.2' class='link' v-for='link in links' :key='link.url'>
        ...
        <ParallaxContainer tag='header' class='press-header video-header'>
            <ParallaxElement :factor='-0.1' tag='h3' class='press-title'>Fire Challenge Winners</ParallaxElement>
        </ParallaxContainer>
            ...
            <figure class='video-thumb' 
                ...
                :key='video.id'>
                    <ParallaxContainer class='video-contain'>
                        ...
                        <span :style='{ backgroundColor: `hsla(${5 + (idx * 6)}, 84%, 65%, 1) `}'>
                            <ParallaxElement :factor='-0.3' tag='img' :src='video.img' :alt='video.title'/>
                        </span>
                    </ParallaxContainer>
            </figure>
        </div>
        ...
        <transition @enter='onModalEnter' @leave='onModalLeave'>
            ...
            <div class='modal' v-if='isVideoActive'>
                ...
                <div class='video-frame' ref='videoFrame'>
                    <div class='close' @click='isVideoActive = false' title='Close Video' ref='modalClose' tabindex='0'>
                    ...
                    <iframe
                        :src='`https://www.youtube.com/embed/${currentYoutubeId}?modestbranding=1&autoplay=1`'
                        ref='modalIframe'
                        frameborder='0' 
                        allow='accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture' 
                        allowfullscreen/>
                    <h5 class='modal-title' ref='modalTitle'>{{currentYoutubeTitle}}</h5>
...
export default {
    ...
    methods: {
        modalSetUp() {
            setTimeout(() => {
                const bgOptions = { movement: -30, target: this.$refs.modalBg.firstElementChild }
                document.addEventListener('mousemove', this.mouseParallax.bind(null, bgOptions))
            }, 1000)
        ...
        mouseParallax ({ movement, target }, {clientX, clientY}) {
            if(target) {
                const x = (clientX - window.innerWidth / 2) / window.innerWidth * movement
                const y = (clientY - window.innerHeight / 2) / window.innerHeight * movement
        ...
        onModalEnter(el, done) {
            ...
            .set([bgPattern, logo, modalIframe, modalClose, modalTitle, modalPlaylist], { opacity: 0 })
            ...
            .add('ui', 1.75)
            .staggerFromTo([logo, modalClose, modalTitle, modalPlaylist], 1, {
                y: 50,
            }, {
                autoAlpha: 1,
                y: 0,
            ...
            .staggerFromTo(modalPlaylist.children, 1, {
                x: 25,
                autoAlpha: 0
            }, {
                autoAlpha: 1,
        ...
        animateSection() {
            ...
            const logos = Array.from(this.$refs.logos.$el.children).filter(el => el.classList.contains('logo'))
            ...
            .staggerFromTo(logos, 0.3, {
                autoAlpha: 0
            }, {
                autoAlpha: 1,
                ease: Sine.easeOut
            }, 0.15, 'logos+=0.3')
            .staggerFromTo(logos, 2, {
                y: 30,            
            }, {
                y: 0,
                ease: Power3.easeOut
            ...
            // Links
            .add('links', 0.7)
            .staggerFromTo(links, 0.3, {
                autoAlpha: 0
            }, {
                autoAlpha: 1,
                ease: Sine.easeOut
            }, 0.15, 'links')
            .staggerFromTo(links, 1.5, {
                y: 15
            }, {
                y: 1,
                ease: Power3.easeOut
...
.logo {
    ...
    &:not(:last-child) {
        margin-right: vw(var(--logo-space));
        &::before {
            content: '';
            position: absolute;
            right: vw(calc(var(--$logo-space)/2));
            bottom: 0;
            top: 0;
...
.video-playlist {
    ...
    @include bp(mobile) {
        ...
        bottom: 0;
        top: initial;
        transform: translate3d(0px, calc(100% + 30px), 0px) rotateY(0deg) rotateX(0deg) !important;
        display: grid;
